...
def _prompt(request: RenderRequest | str, context: str = "") -> str:
    ...
    if request.mode == "designed_card":
        common = (
            ...
            "DESIGN REFERENCE ONLY: the separately supplied template is inspiration only (inspiration-only) and never copy it exactly. Ignore every word, letter, number, logo, label, and caption in it; "
            "never read, reproduce, or copy its visible text, house, coordinates, or distinctive details. The only visible text allowed in the final card "
            "is the exact card_brief text. Use the supplied listing assets only for their declared roles and do not reproduce incidental source-image text. "
